...
import { Music, Pause, ChevronUp } from "lucide-react";
...
export default function WritePlaylistDock() {
  ...
  return (
    <div dir="rtl" className="fixed z-40 bottom-[136px] left-[max(1rem,calc(50%-12.5rem))]">
      ...
      <div className="flex items-center h-11 rounded-full bg-[#16161A] text-[#F1F0EC] shadow-[0_4px_14px_rgba(0,0,0,0.18)]">
        ...
        {started && (
          ...
          >
            <ChevronUp className={`w-4 h-4 text-[#A8A69D] transition-transform ${open ? "rotate-180" : ""}`} strokeWidth={2} />
          </button>
        )}
      </div>
    </div>
